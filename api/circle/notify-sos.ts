...
// separate table from ai_request_log. SOS pushes fan out to every active
// circle friend, so the cap here is deliberately the tightest of the
// circle/* endpoints.
async function checkAndRecordRateLimit(
  ...
  try {
    const countRes = await fetch(
      `${supabaseUrl}/rest/v1/action_rate_log?action=eq.notify-sos&subject=eq.${encodeURIComponent(subject)}&created_at=gte.${encodeURIComponent(since)}&select=id`,
      {
        method: "HEAD",
    ...
    await fetch(`${supabaseUrl}/rest/v1/action_rate_log`, {
      ...
      body: JSON.stringify({ action: "notify-sos", subject }),
    })
    fetch(
      `${supabaseUrl}/rest/v1/action_rate_log?action=eq.notify-sos&subject=eq.${encodeURIComponent(subject)}&created_at=lt.${encodeURIComponent(since)}`,
