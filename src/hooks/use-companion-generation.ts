...
// still waiting for it when Home mounts (see src/lib/companion-generation.ts
...
export function useCompanionGeneration(): void {
  ...
  useEffect(() => {
    ...
    runCompanionObservationGenerationOncePerDay({ userId, aiEnabled, language }).catch((err) => {
